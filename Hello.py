...
def main():
    ...
    if opcao_selecionada == 'editar':
        ...
        if not df.empty:
           ...
           # Definir o valor para dias_para_fim_vigencia_edit
            ...
            numero_contrato_edit = st.text_input('NRO CONTRATO', value=df.loc[indice_edicao, 'NRO CONTRATO'])
    ...
    st.write(tempo_entrada.sort_values(by='Dias Decorridos', ascending=False).drop(columns=['DATA DE ENTRADA'], inplace=True))
