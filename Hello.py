...
def main():
    ...
    if exibir_formulario_edicao:
        ...
        if not df.empty:
            ...
            data_expiracao_contratual_edit = st.date_input('Data Expiração Contratual*', value=datetime.strptime(df.loc[indice_edicao, 'DATA EXPIRAÇÃO CONTRATUAL'], '%d/%m/%Y').strftime('%d/%m/%Y'), format='DD/MM/YYYY')
